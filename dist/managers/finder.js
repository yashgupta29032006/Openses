...
exports.getFinderState = getFinderState;
exports.restoreFinder = restoreFinder;
const applescript_1 = require("../utils/applescript");
async function getFinderState() {
...
async function restoreFinder(state) {
    // Activate Finder
    // Close existing windows? Maybe not, just open the saved ones.
    for (const win of state.windows) {
        const folderPath = win.title;
        // Open folder
        await (0, applescript_1.runScript)(`tell application "Finder" to make new Finder window to (POSIX file "${folderPath}")`);
        // Set bounds
        ...
        await (0, applescript_1.runScript)(`tell application "Finder" to set bounds of window 1 to {${L}, ${T}, ${R}, ${B}}`);
    }
}
