import { AppProcess } from './interfaces';
...
export class RelevanceEngine {
    private static MIN_MEMORY_MB = 50;  // Filter out tiny helpers
    private static MIN_CPU_PERCENT = 0.1;

    static shouldExclude(app: AppProcess): boolean {
        if (!app.bundleId) return true; // No bundle ID = probably not a user app
